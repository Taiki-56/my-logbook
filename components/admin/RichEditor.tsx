...
import { EditorContent, JSONContent, useEditor } from "@tiptap/react"; //* 追加: JSONContent をインポート
...
const editorExtensions = [
  ...
  Image,
  // Link.configure({ openOnClick: false }), // StarterKitに統合
...
const RichEditor = ({ initialContent, onChange }: RichEditorProps) => {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "tiptap-content prose prose-neutral max-w-none focus:outline-none min-h-[500px] p-8 font-['Liberation_Serif:Regular'] text-[#1b1c1c] leading-relaxed " +
  ...
  return (
    <div className="bg-white border border-[#c1c6d7] rounded shadow-sm flex flex-col">
      {/* ========================================================= */}
      {/* Chronicle Professional Toolbar                            */}
      {/* ========================================================= */}
      <div className="flex flex-wrap items-center gap-1 p-2 bg-[#fbf9f8] border-b border-[#c1c6d7] rounded-t text-[#414754]">
        {/* 見出しグループ */}
        <button
          type="button"
