  ...
  secondary: [
    {
      ...
      description:
        "M2 Mac上でプライバシーを保ちながら高速な推論を行うためのモデル選定と環境設定の完全マニ",
      tags: ["AI", "Mac"],
    ...
    },
    {
      ...
      description:
        "バンクーバーでのソフトウェアエンジニアとしての働き方と、日々の言語学習アプローチの変遷。",
      tags: ["Canada", "English"],
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="flex flex-col gap-4 lg:hidden">
        {[featuredArticles.main, ...featuredArticles.secondary].map(
          (article) => (
            <Link
              key={article.id}
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex gap-1 items-center mb-1">
                  <span className="font-['Noto_Sans_JP'] font-normal text-[11px] text-[#707581] leading-normal">
                    {article.date || "11月 5日"} •{" "}
                    {article.readTime?.replace(" read", "") || "8分"}
                  </span>
                </div>
      ...
      <div className="hidden lg:grid lg:grid-cols-2 gap-4 w-full">
        ...
        <Link
          href={`/article/${featuredArticles.main.slug}`}
        ...
        >
          ...
          <div className="lg:w-1/2 p-6 flex flex-col justify-center">
            ...
            {/* Title */}
            <h3 className="text-2xl text-[#1b1c1c] leading-[31.2px] mb-3">
              {featuredArticles.main.title}
            </h3>

            {/* Description */}
            <p className="text-base text-[#414754] leading-6 mb-4 line-clamp-3">
              {featuredArticles.main.description}
            </p>
            ...
            <div className="flex gap-2 flex-wrap">
              {featuredArticles.main.tags.map((tag) => (
                <span
                  key={tag}
                  className="bg-[#f5f3f3] border border-[#c1c6d7] rounded-sm px-2.5 py-1.5 font-['JetBrains_Mono'] font-normal text-xs text-[#414754] leading-4"
                >
                  {tag}
                </span>
              ))}
        ...
        {featuredArticles.secondary.map((article) => (
          ...
          >
            ...
            {/* Title */}
            <h3 className="text-lg text-[#1b1c1c] leading-7 mb-2">
              {article.title}
            </h3>

            {/* Description */}
            <p className="text-sm text-[#414754] leading-5 mb-4 line-clamp-2">
              {article.description}
            </p>
