...
/** Renders the post: title, meta info, thumbnail, and the post's HTML body. */
const PostContent = async ({ post, displayContent }: Props) => {
  const t = await getTranslations("Posts.slug.postContent");
      ...
      <div className="px-4 pb-8 sm:px-6 sm:pb-10 md:px-10 md:pb-12 lg:px-12">
        <div
          className="prose max-w-none font-['Noto_Sans_JP'] text-[#2d313a]
                     ...
                     /* Blockquote (summary box) and code block styles */
          ...
          dangerouslySetInnerHTML={{ __html: displayContent.html || `<p>${t("noContent")}</p>` }}
        />
      </div>
    </article>
  );
